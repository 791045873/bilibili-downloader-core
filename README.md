...
如果你只想单独补齐视觉代理依赖，或者 `pnpm install` 时因为本机没有 Python 而跳过了这一步，可以在仓库根目录执行 `pnpm setup:vision-proxy`。该命令会自动寻找可用的 Python 解释器，在 `packages/vision-proxy/.venv` 创建虚拟环境并安装 `dashscope`、`python-dotenv` 等锁定版本依赖。
...
如果想要在 NAS 中使用，在仓库根目录执行 `pnpm docker:build` 构建两个镜像（tag 携带各自包 version，如 `bilibili-downloader:0.0.1` 与 `bilibili-downloader:vision-proxy-0.0.1`），再执行 `pnpm docker:run` 通过 docker compose 启动 `server`（Node + 前端 + FFmpeg，对外暴露 3000 端口）与 `vision-proxy`（Python 视觉代理，仅供容器间调用）。两个容器均配置 `restart: unless-stopped`，任一崩溃会自动单独重启。两个镜像也可用 `pnpm --filter @bilibili-downloader/docker docker:build:server` / `docker:build:vision-proxy` 各自独立构建。
...
如需启用 AI 视频分析视觉代理，先在**前端设置页**配置大模型 API Key/模型（存入数据库），Node 会把 `llm.apiKey` 经 `Authorization` 头传给 vision-proxy 容器；「测试连接」与代理同用写死的原生 DashScope 端点，无需配置模型端点。
