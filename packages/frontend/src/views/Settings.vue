...
<template>
  <div class="space-y-6">
    <div class="rounded-lg border border-zinc-200 bg-white p-6">
      <h2 class="text-lg font-semibold text-rose-600 mb-6">下载设置</h2>
        ...
        <div class="rounded-md border border-zinc-200 bg-zinc-50 p-4">
          <div v-if="downloadConfig" class="space-y-3">
            <div class="flex items-center justify-between gap-3">
              <span class="text-sm text-zinc-700">服务端下载根目录</span>
              <span class="rounded border border-zinc-300 px-2 py-0.5 text-xs text-zinc-600">
                {{ downloadConfig.source === "env" ? "环境变量" : "默认目录" }}
              </span>
            </div>
            <div class="flex items-center gap-2">
              <code class="min-w-0 flex-1 break-all rounded bg-zinc-100 px-3 py-2 text-xs text-zinc-700">
                {{ downloadConfig.outputDir }}
              </code>
              <button
                class="shrink-0 rounded-md border border-zinc-300 px-3 py-2 text-xs text-zinc-700 hover:border-rose-500 hover:text-rose-600 transition-colors"
                @click="copyOutputDir"
              >
                {{ copied ? "已复制" : "复制" }}
              </button>
            </div>
            <p v-if="copyError" class="text-xs text-amber-600">复制失败，请手动选择路径文本。</p>
            <p class="text-xs text-zinc-500">入队时填写的是该目录下的相对子目录。</p>
          </div>
          <p v-else-if="downloadConfigError" class="text-sm text-red-600">{{ downloadConfigError }}</p>
      ...
      <div class="mb-6">
        ...
        <div class="space-y-3">
          <div class="flex items-center justify-between py-2">
            <span class="text-sm text-zinc-700">视频画质</span>
            <select
              v-model.number="settingsStore.settings.defaultQuality"
              class="rounded border border-zinc-300 bg-white px-3 py-1.5 text-sm text-zinc-800 focus:border-rose-500 focus:outline-none"
            >
          ...
          <div class="flex items-center justify-between py-2">
            <span class="text-sm text-zinc-700">视频编码</span>
            <select
          ...
          <div class="flex items-center justify-between py-2">
            <span class="text-sm text-zinc-700">音频质量</span>
            <select
