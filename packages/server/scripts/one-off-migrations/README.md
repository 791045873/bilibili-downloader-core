...
| `004-outputfile-relative.sql` | task."outputFile" 与 analysis_sub_task.output_file 绝对路径改写为相对下载根目录的相对路径 | **按环境手动执行、幂等可重复**：用法同 003；重跑 `migrate-sqlite-to-postgres.mjs` 后需重跑本脚本 |

**001/002 勿在任何库重复执行。** 若存在未执行过它们的异常库（理论上不存在），人工核对后单独执行。003/004 例外：幂等设计，可安全重复执行。
