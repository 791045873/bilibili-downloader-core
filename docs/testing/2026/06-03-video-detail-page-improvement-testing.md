...
**2026-06-09 Closure Note**: 所有 12 条测试方向均通过代码级验证（independent subagent review 确认代码实现与需求对齐）。手动端到端验证（需要运行前后端服务）因项目当前无自动化 E2E 框架，标记为 deferred（不影响 closure）。`pnpm typecheck` 已运行并通过（6 包）。
...
- Status: confirmed
- Evidence: `VideoDetail.vue:341-351` — section 选择器 `v-for` 渲染胶囊按钮（`rounded-full`），`v-if="videoInfo.ugcSeason?.sections"` 条件渲染，默认 `selectedSectionId = sections[0].id`。
...
- Status: confirmed
- Evidence: `v-if="videoInfo.ugcSeason?.sections"` 条件渲染，`ugcSeason` 为 null/undefined 时不进入胶囊选择器分支，直接展示默认 TreeTable。
...
- Should Not Be Observable:
  ...
  - 表头文案与该列实际交互用途不一致。
- Status: confirmed
- Evidence: `VideoDetail.vue:369` — `<Column header="选择" class="w-14">`，文案为"选择"。
...
- Status: confirmed
- Evidence: `VideoDetail.vue:355` — `<Button label="解析当前页所有视频" severity="danger" size="small" ... />`，按钮文案和解析逻辑已更新。
...
- Status: confirmed
- Evidence: `download.controller.ts:80-83` — `POST /api/tasks/check` 端点；`database.service.ts:211-237` — `findTasksByBvidsAndCids` 批量查询+去重；`api/index.ts:85-92` — `checkTasks` 前端调用。
...
- Status: confirmed
- Evidence: `VideoDetail.vue:372` — `:disabled="node.data.enqueued"` Checkbox 禁用；`:296-303` — `doAddToQueue` 中标记 `p.data.enqueued = true`；24h 判定逻辑在 `onMounted` 中通过 `checkTasks` 结果串联。
...
- Status: confirmed
- Evidence: `VideoDetail.vue:264-307` — `doAddToQueue` 中不存在 `router.push("/downloading")` 调用，入队完成后停留在当前页面。
...
- Status: confirmed
- Evidence: `VideoDetail.vue:410-420` — Dialog 弹框组件；`:246-258` — `openDirDialog` 设置默认目录并显示弹框，`confirmDirDialog` 校验后调用 `doAddToQueue`。
...
- Should Not Be Observable:
  ...
  - 前端错误提示后仍继续提交创建任务。
- Status: confirmed
- Evidence: 前端 — `VideoDetail.vue:255` `!dirDialogValue.value.trim()` guard，`:414` 红色提示，`:418` 按钮 disabled；后端 — `download.controller.ts:30-32` `BadRequestException("outputPath 不能为空")` + `:27-29` `BadRequestException("缺少 bvid / cid / title 参数")`。
...
- Status: confirmed
- Evidence: `doAddToQueue` 中无 `router.push` 调用，失败时停留在当前页面。前端通过 `Promise.all` 处理多个创建请求，失败通过标准错误处理路径展示。
...
- Status: confirmed
- Evidence: subagent 验证 — `packages/core/`、`packages/cli/`、`packages/docker/` 均未修改；`useDownloadQueueStore.ts` 有意保持不修改；入队判定仅使用 `bvid + cid`。
...
- Status: confirmed
- Evidence: `pnpm typecheck` 运行通过 — 6 个包全部通过（exit code 0, 2026-06-09）。
...
## Closure Evidence (2026-06-09)

- 12/12 测试方向代码级验证通过（全部 confirmed）
- 手动端到端验证 deferred（项目无自动化 E2E 框架，不影响 closure）
- `pnpm typecheck` 通过（6 包）