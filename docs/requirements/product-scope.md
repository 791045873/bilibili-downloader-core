...
| 功能 | 优先级 | 状态 |
|------|--------|------|
| 视频解析页面优化 | P0 | done — 已完成实施（section 选择器、一键解析、入队去重、目录弹框、空目录校验） |
| UI 界面优化 | P0 | deprecated — 已废弃，需重写需求（涉及交互调整和后端接口修改） |
| 批量添加待下载视频（延迟解析） | P1 | needs-requirement — 待编写需求 |
| 下载目录指定与查看 | P1 | needs-requirement — 待编写需求 |
