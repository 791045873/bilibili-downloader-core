...
- 日期：2026-08-12
- 状态：draft（已经过一轮独立评审并修订；评审通过并进入计划阶段后才可实施）
- 来源：用户直接需求（2026-08-12）：1) 接口级缓存，同一接口 24 小时内复用缓存；2) 接口 code 返回 -412 时自动重试，最多 5 次，超出后调用失败
- 影响包：`packages/bilibili-api-sdk/`（下游 `packages/adapters/` 无需改动）
...
1. **接口级缓存**：同一接口（相同 URL + 相同业务参数 + 相同登录身份）在 24 小时内复用缓存响应，默认开启。
2. **-412 自动重试**：接口返回 code = -412（含 HTTP 412）时自动重试，最多重试 5 次；仍失败则本次接口调用以 `BiliError(-412)` 失败。

## In Scope

### 1. 缓存（`BaseApi.request` 层，GET 读接口）

- 新增 `ApiCacheStore` 接口与默认内存实现（`Map` + TTL + 容量上限 + 插入顺序淘汰），不引入新依赖。缓存为 per-client 实例内存，多 client 实例间不共享（server 的 parse/download 各持一个 client，属预期行为）。
...
- 配置入口：`ClientOptions.cache`（`enabled` 默认 true、`ttlMs` 默认 24h、`maxEntries` 默认 500、可注入自定义 `ApiCacheStore`）。
...
- 次数语义：首次尝试 + 最多 5 次重试 = 最多 6 次请求；`maxRetries` 默认 5，可配置。
- 退避策略：指数退避 `baseDelayMs × 2^n`（`baseDelayMs` 默认 1000）+ 0~250ms 随机抖动，即约 1s/2s/4s/8s/16s。风控场景立即重试无意义且加重风控。
...
- http 层网络重试保持不变，作为内层重试。单次调用最坏请求数：WBI GET 接口为 `(1 + httpRetries) × 2（v_voucher） × (1 + 5) = 24` 次；`postForm` 无 v_voucher 逻辑，为 `(1 + httpRetries) × (1 + 5) = 12` 次（均按默认 httpRetries=1 计）。

## Out Of Scope

- 磁盘持久化缓存（进程重启后保留）。v1 仅内存缓存，存储接口为未来持久化预留。
...
- 下游 `packages/adapters/`、`packages/server/` 的任何改动。
...
3. 重试成功则正常返回数据（并按缓存规则写缓存）；持续 -412 则按 2s/4s/8s/16s 退避继续重试。
4. 第 5 次重试仍 -412，SDK 抛出 `BiliError(-412)`，调用方按现有失败路径处理。
...
- 缓存与重试均为 SDK 内建行为，对 `adapters`/`server` 调用方透明，不传新参数即获得默认行为。
...
- **内存增长**：`maxEntries` 上限 + 插入顺序淘汰兜底；per-client 实例，进程重启自然清空。
- **重试与内层重试叠加**：WBI GET 最坏 24 次请求、单次调用最长阻塞约 31s 退避 + 请求耗时；调用方如需更快失败可调低 `maxRetries`。
- **现有测试可能受影响**：缓存默认开启会改变"同参数重复调用"的测试行为，实施时需检查并适配现有 vitest 用例。

## Open Questions

1. **（进入计划阶段前需确认）**"最多 5 次"的语义：本方案按"首次 + 5 次重试 = 最多 6 次请求"实现（`maxRetries = 5`）。若意图是"总共最多 5 次请求"，仅需调整默认值语义，不影响设计。
2. 高变化端点的 24h 缓存接受度：本方案按需求字面统一 24h 默认 + 单次可覆盖。需知悉的下游影响：`video.view` 的 `pages`/`ugc_season`（新分 P 24h 内不可见）、`user.spaceArcSearch`/`seasonsSeriesList`（新投稿不可见）、`favorite` 列表、`history`、`search`、`followings`、`playerV2` 字幕等。若不接受，备选是分级 TTL（列表/元数据类 5~30 分钟，静态类 24h），配置机制已支持，仅需调整内置默认值。
3. **（进入计划阶段前需确认）**PGC playurl（bangumi/cheese）是下游下载核心路径且易触发风控，但绕过 `BaseApi.request`，本期 -412 重试对它无效。选择：a) 本期显式不覆盖（推荐，最小切片）；b) 将 PGC playurl 收敛到 `request()` 或单独包装重试。`search.suggest` 不解包不抛错，同样不覆盖。
4. playurl 是"排除缓存"还是"给短 TTL（如 30 分钟）"？本方案默认排除，倾向保守。
...
6. 接口连续返回 `code: -412` 时，自动重试且目标端点的请求次数为 6（1 + 5，用 mockFetch `byUrl` 仅计目标端点，WBI nav、spi/GenWebTicket 等辅助请求不计入），最终抛 `BiliError(-412)`；第 N 次（N ≤ 5）重试成功则正常返回且目标端点请求数为 N + 1。
...
- 新增：`src/cache/`（`ApiCacheStore` 接口 + 内存实现 + key 序列化），约 1 个模块。
