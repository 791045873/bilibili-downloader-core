...
- Core package (`packages/core/`) must not depend on any UI framework, CLI framework, or server framework.
- Adapters package (`packages/adapters/`) implements ports defined in Core.
- CLI, Server, and Docker are runtime entry points that compose Core + Adapters.
- Dependency direction: Runtimes → Adapters → Core (never reverse).
- Package imports: use pnpm workspace protocol (`workspace:*`) for internal dependencies.
