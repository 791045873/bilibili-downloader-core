...
| Server       | `packages/server/src/`        | NestJS 后端 API，下载任务管理、视频分析编排、全局请求日志；持久化为 PostgreSQL（`pg` 连接池，经 `DATABASE_URL`），`database.service.ts` 为唯一数据访问层（Prisma 8 基础设施已接入：`database/prisma.service.ts` + `src/prisma/contract.*`，尚无消费者；contract 变更后跑 `pnpm --filter @bilibili-downloader/server prisma:emit`），一次性迁移脚本在 `packages/server/scripts/migrate-sqlite-to-postgres.mjs`；数据层行为测试在 `packages/server/tests/`（vitest，需 `TEST_DATABASE_URL`）；知识发布管道在 `packages/server/src/knowledge/`（`KnowledgePublisherService` 上传截图至腾讯云 COS 并写 `summary`/`summary_segment`，`CosStoreService` 封装 `cos-nodejs-sdk-v5`） | 2026-09-01    | high       |
